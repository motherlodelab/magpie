...
// TestAdapterEndpointJoin pins the wire path per adapter and base shape.
// The Anthropic side joins tolerantly: bases in the wild disagree about
// carrying the /v1 segment (api.anthropic.com does not; the zen bases and
// MAGPIE_BASE_URL usually do), and a doubled segment —
// opencode.ai/zen/go/v1/v1/messages — 404'd every messages-routed zen
// model (claude-*, minimax-*, qwen-*-max). The OpenAI side deliberately
// appends /chat/completions verbatim: its bases carry their own path
// prefix, and root-mounted gateways must keep working untouched.
func TestAdapterEndpointJoin(t *testing.T) {
	...
	tests := []struct {
		name     string
		envelope string
		suffix   string // appended to the httptest base URL
		build    func(base string) extract.Extractor
		wantPath string
	}{
		{
			name:     "anthropic base without /v1 (api.anthropic.com shape)",
			envelope: anthropicEnvelope(`{"name":"Widget","price":12.99}`),
			build:    func(base string) extract.Extractor { return extract.NewAnthropic(base, "k", "claude-sonnet-5", sch) },
			wantPath: "/v1/messages",
		},
		{
			name:     "anthropic base with /v1 (zen shape, was /v1/v1/messages)",
			envelope: anthropicEnvelope(`{"name":"Widget","price":12.99}`),
			suffix:   "/v1",
			build:    func(base string) extract.Extractor { return extract.NewAnthropic(base, "k", "claude-sonnet-5", sch) },
			wantPath: "/v1/messages",
		...
		{
			name:     "openai base with /v1 (api.openai.com and zen shape)",
			envelope: openAIEnvelope(`{"name":"Widget","price":12.99}`),
			suffix:   "/v1",
			build:    func(base string) extract.Extractor { return extract.NewOpenAI(base, "k", "gpt-4o-mini", sch) },
			wantPath: "/v1/chat/completions",
		},
		{
			name:     "openai base without /v1 stays root-mounted",
			envelope: openAIEnvelope(`{"name":"Widget","price":12.99}`),
			build:    func(base string) extract.Extractor { return extract.NewOpenAI(base, "k", "gpt-4o-mini", sch) },
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				path = r.URL.Path
				w.Header().Set("Content-Type", "application/json")
				_, _ = io.WriteString(w, tt.envelope) //nolint:errcheck // fake provider: write failure fails the request, not the test
			}))
			t.Cleanup(srv.Close)
			if _, err := tt.build(srv.URL+tt.suffix).Extract(t.Context(), extract.ExtractInput{Markdown: "# Widget\n\nPrice: 12.99"}); err != nil {
				t.Fatalf("Extract: %v", err)
			}
			if path != tt.wantPath {
